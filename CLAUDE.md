...
- `npm run gen:cover <slug> [--style line-art|conceptual] [--concept "..."] [--force]` — generate one post's AI cover via the image-gateway. Needs `IMAGE_GATEWAY_URL` + `IMAGE_GATEWAY_TOKEN` in env / `.env.local`; fails only when invoked, never at build.
...
deterministic, article-seeded branded fallback). It writes
...
backfill, a **non-fatal** hook in the Plan B agent's draft step, and the
`Attach blog cover` workflow used by external drafting services.

This repository is the source of truth for cover prompts, style selection, text
detection, fallback rendering, frontmatter mutation, and asset paths. External
services dispatch `.github/workflows/attach-cover.yml`; do not duplicate this
pipeline elsewhere.
...
rendering remain local (resvg/Satori); the site build never calls the gateway.
