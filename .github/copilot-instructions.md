...
- Use Node.js 24 or newer and npm; clean installs use `npm ci`.
...
- Format changed files with `npx prettier --write <paths>`; check formatting with `npx prettier --check .`. Prettier uses `prettier-plugin-astro`.
