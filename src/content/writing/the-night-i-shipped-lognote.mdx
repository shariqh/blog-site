...
The fix is the part I am proud of with a cross-process model-load lock so two engines can never race for the same memory. We added a RAM gate that picks the largest model that actually fits the machine in front of it and added in-flight guard that greys the button when you press it. Five hours in I had three small gaps closed. And the detail that mattered most to me: the recording and transcription path, the heart of the product, never failed. The mic and system-audio tap held while WhisperKit kept doing its job on device. The worst problem of the day came from Import, the first non-core feature.
