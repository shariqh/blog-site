---
title: 'Hooks, not vibes: deterministic guard-rails for Claude Code'
date: '2026-06-29'
draft: false
tags:
  - ai
...
summary: >-
  ...
  moved seven of lognote's rules into Claude Code's PreToolUse and PostToolUse
  hooks, and learned where determinism stops.
hero:
  image: /static/images/blog/hooks-not-vibes-deterministic-guard-rails/cover.png
  alt: >-
    Cover illustration for "Hooks, not vibes: deterministic guard-rails for
    Claude Code".
  prompt: >-
    ...
    Concept: Hooks, not vibes: deterministic guard-rails for Claude Code.
    The rules you give a coding agent live as prose in a config file and rely on
    it remembering them every turn. That breaks when the context summarizes. I
...
So I stopped trusting memory and moved the rules into Claude Code hooks.

## What a Claude Code hook can and cannot do
...
## The seven Claude Code hooks

I built seven hooks for [Oris](https://orisnotes.com/?utm_source=shariq.dev&utm_medium=blog&utm_campaign=hooks), back when it was still [called lognote](/blog/the-night-i-shipped-lognote).
...
That loop, an independent model attacking my work the instant it exists, is an awesome payoff. The hook did not fix anything itself but it guaranteed the review happened on every PR no matter what state the context was in. I later measured that same review-gate coverage across 75 agent-labeled PRs in a two-week window, not just this one PR, in [a breakdown of Oris's review-gate numbers](/blog/ai-became-my-operating-model).
