...
Running an AI-assisted software development workflow at that volume raises a different question: not how much got automated, but whether the control system kept pace with the unit of delegation.
...
These review gates don't replace judgment. They filter signal from noise and surface things that warrant attention before I decide what to look at closely. When the volume is 75 agent PRs in two weeks, I need that layer to be reliable and consistent. Some of that reliability comes from wiring the gates as [deterministic Claude Code hooks](/blog/hooks-not-vibes-deterministic-guard-rails) instead of instructions I hope the agent remembers.
...
**Decision ownership.** Who's writing the spec? Who decides what merges? Track those explicitly rather than leaving them as implicit assumptions. When terminal scroll made that tracking impossible to keep straight across repos, I built [a dedicated inbox for exactly that](/blog/terminal-scroll-is-not-a-workflow).
